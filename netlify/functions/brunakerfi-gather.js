...
async function ktByCompanyName(company, address) {
  ...
  if (!_ktByNameCache) {
    ...
    for (let from = 0; ; from += 1000) {
      const r = await fetch(`${SUPABASE_URL}/rest/v1/fyrirtaeki?deleted_at=is.null&kennitala=not.is.null&select=nafn,kennitala,heimilisfang&order=id.asc`, { headers: sbHeaders({ Range: `${from}-${from + 999}`, 'Range-Unit': 'items' }) });
