...
## 9 · Hreinsun 2026-08-11 — hvað var gert og hvað var VÍSVITANDI EKKI gert

Beiðni Agnars: „cleanup. for emty files. fake links. dead links. autogenerated
false files". Mælt fyrst, hreinsað svo — og tvennt stöðvað af því mælingin sagði
að hreinsun hefði EYÐILAGT gögn.

### Gert

| Aðgerð | Fjöldi | Athugasemd |
|---|---|---|
| `.html` Drive-redirect stubbar **umbreyttir** í alvöru Drive-viðhengi | 63 (45 í blobbi, 43 staðir) | `external:true` + `drive_id` — sama form og 369 lögleg Drive-viðhengi höfðu fyrir |
| Skjalaröð sem vísaði á 0-bæta skrá | 1 | falskt „skjal á skrá" |
| Nákvæm afrit í `customer_documents` | 7 | pör losuð fyrst (4) svo FK-in hafnaði ekki |

`customer_documents` 3.638 → 3.630. Viðhengi 1.314 → 1.314 (**ekkert tapaðist**).
Drive-viðhengi 369 → 414. Engin pör með dauða tilvísun eftir á.
Afrit: `backup_20260811_customer_documents`, `backup_20260811_app_settings`,
`backup_20260811_storage_junk`.

⚠️ **Stubbarnir voru EKKI bara rusl.** Þeir eru 331–410 bæta HTML-skrár sem
beina á Drive (`<meta http-equiv="refresh">`), en **39 af 45 Drive-auðkennum í
þeim voru hvergi annars staðar skráð**. Að eyða þeim beint hefði eytt einu
tilvísuninni í 39 skjöl. Þess vegna UMBREYTT, ekki eytt.

Storage-hlutirnir sjálfir (63 html + 7 tómir PDF, ~22 kB) standa eftir sem
ótengdir bætir — `storage.protect_delete()` bannar SQL-eyðingu og krefst
Storage-API. Þeir sjást hvergi í viðmótinu lengur.

### VÍSVITANDI EKKI GERT

1. **Ekki eytt út frá „793 dauðum hlekkjum".** Kafli 2 hér að ofan segir sjálfur
   að sú tala sé login: samningar-masterinn telur 0 skrár, svo 332 samningar eru
   taldir dauðir að ósekju. Fjöldaeyðing á þeim grunni hefði eytt allt að 332
   góðum samningsskjölum. **Fasi 1 þarf að koma á undan hvers kyns eyðingu.**
2. **Ekki eytt 430 draugaröðum.** 239 eru reikningaskráningar (202 með
   reikningsnúmer, 87 í pörum) og 188 eru skoðanaskrár. Þær eru skrár um
   RAUNVERULEGA hluti sem eiga bara ekki PDF — ekki tilbúningur. Að eyða þeim
   eyðir bókhalds- og skoðunarsögu.
3. **Ekki tengt 80 hangandi þjónustusamninga sjálfvirkt.** Sjá næsta kafla.

### Þjónustusamningar — af hverju þeir tengjast ekki

358 `samningur`-raðir: 268 tengdar STAÐ, 10 aðeins kt, **80 tengdar ENGU**
(hvorki `fyrirtaeki_id` né `customer_base_id`) þótt 351 eigi alvöru skrá.
Þessir 80 birtast því á engum prófíl.

Allir 80 eru **Drive-only** (ekkert `storage_path`), svo slóð-ber-staðinn
bragðið virkar ekki — það er einmitt það sem gerir Supabase-leiðina rétta.
Eina auðkennið er heitið, og heitin eru skjalatitlar en ekki fyrirtækjanöfn:
„Þjónustusamningur BF17", „RESOLVE: þjónustusamningur", „slökkvitækja (ómerkt)".

Mælt: af 75 nothæfum heitum mátaðist **1** ótvírætt við fyrirtæki (0 margræð,
74 engin). Sjálfvirk nafna-tenging er því ágiskun — nákvæmlega það sem
`document_pairs`-kaflinn í CLAUDE.md varar við. **Þessir 80 þurfa Fasa 2
(innihalds-lestur) eða handvirka tengingu; þeir leysast ekki úr gögnunum eins
og þau standa.**
